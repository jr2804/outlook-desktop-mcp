...
async def test_create_event_blank_subject_rejected():
...
async def test_create_event_whitespace_subject_rejected():
...
async def test_create_event_valid_subject_proceeds():
    ...
    result = await server.create_event(
        subject="Valid test event", start="2026-05-15 09:00", end="2026-05-15 18:00"
    )
    # Should NOT be the blank-subject error
    if result.startswith("{"):
        data = json.loads(result)
        if "error" in data:
            assert "blank" not in data["error"].lower(), \
                f"Should not be blank-subject error: {data['error']!r}"
    print(f"  PASS  create_event(valid subject) -> bridge proceeds (result={result[:60]!r})")


async def test_create_meeting_blank_subject_rejected():
    """create_meeting with subject='' should be rejected."""
    from outlook_desktop_mcp import server
    result = await server.create_meeting(
        subject="", start="2026-05-15 09:00", end="2026-05-15 18:00",
        required_attendees="test@example.com"
    )
    data = json.loads(result)
    assert "error" in data, f"Expected error, got: {result!r}"
...
async def test_create_task_blank_subject_rejected():
...
async def test_create_task_whitespace_subject_rejected():
    """create_task with subject='\t\n  ' should be rejected."""
    from outlook_desktop_mcp import server
    result = await server.create_task(subject="\t\n  ")
    data = json.loads(result)
...
async def test_update_event_whitespace_subject_rejected():
...
async def test_update_event_no_subject_allowed():
    ...
    if result.startswith("{"):
        ...
        if "error" in data:
            assert "blank" not in data["error"].lower(), \
                f"Empty string = 'skip' should be allowed, got: {data['error']!r}"
